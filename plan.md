...
- [~] Products now read and mutate real products, category assignment,
      ...
      values, and variants through the protected admin adapter. Variant
      deployment proof, custom-field controls, media upload, and delete-blocker
      evidence remain before completion.
...
| 2026-07-20 | Implemented live Product variant controls inside the Businesses workspace.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | The Products tab now lists real variants for the selected saved product, creates/edits exact SKU/price/stock/availability combinations through `save_admin_product_variant`, deletes variants through an in-page confirmation panel using `delete_admin_product_variant`, and feeds the existing backend validator one selected value per product option. Release marker updated to `business-products-variants-live-v1`. Local gates passed: focused ESLint, `npm run typecheck`, `npm test` (`115/115`), `npm run test:wa` (`16/16`), `git diff --check`, and `npm run build`. Commit, deployment, authenticated production browser mutation smoke, and cleanup evidence remain before this slice is accepted. |
