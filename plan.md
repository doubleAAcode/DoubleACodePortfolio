...
- [~] Catalog Routes now reads, creates, edits, activates/deactivates, orders,
      and deletes real browse groups through protected admin APIs. Production
      browser evidence confirms read, create, and edit. Deletion works through
      the protected backend path, but the UI still needs a non-native
      confirmation panel before the tab graduates to complete.
