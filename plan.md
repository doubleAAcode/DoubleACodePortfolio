...
Current authorized next action: finish 1C Human operations with conversation
lifecycle and collaboration commands, then perform the controlled human-send
activation. The durable retry processor now claims only definitive retryable
failures, rechecks the customer-service window, quarantines expired ambiguous
sends, and exposes audited internal-admin reconciliation. The provider-send
switch and scheduler remain unactivated until this release reaches production;
inbox UI promotion remains 1D.
