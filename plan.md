...
verified. The connected production Live Test attempted the approved
`hello_world` template to the exact user-confirmed recipient, and Meta returned
`#131030` because that number is not yet in the test number's allowed recipient
list; the failed attempt is durably visible in the provider event timeline. Add
and verify that exact recipient in Meta API Setup, resend the template from Live
Test, reply with marker `LIVE-1A-0717-A2` in that exact thread, and record tenant
routing, durable inbound, duplicate/retry behavior, deterministic response, and
status-hook evidence. Use the connected Live Test and database evidence;
...
- [~] Use the exact Flow Manager Live Test route as the repeatable provider
      harness. Real connection data, approved-template opening sends, restart
      sends, and event refresh are connected; production roundtrip evidence is
      still required before the route or 1A is complete.
