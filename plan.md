...
- [x] Route Values reads and mutates real group values and their product
      ...
      confirmation.
- [x] Products now read and mutate real products, category assignment,
      ...
      variant, option, and option-value delete blockers protect order history
      and variant integrity in the shared catalog action layer.
- [~] Checkout Settings now reads and mutates real fulfillment policy, checkout
      prompts, confirmation copy, delivery areas, pickup locations, payment
      methods, runtime skip behavior, and owner-approval policy through
      protected admin adapters. A conversation-engine regression now proves the
      deterministic checkout runtime consumes business checkout areas, payment
      methods, and prompts. Remaining proof before completion is confirming a
      production WhatsApp order flow consumes the saved checkout choices.
...
| 2026-07-20 | Completed the Business Products tab from exact commit `26ddb99`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Production returned `200` with `X-Connect-Release: business-products-delete-blockers-live-v1` and capability `business-products-delete-blockers-live`. Authenticated browser verification on `/connect/admin/businesses/double-a-test-business/products` used temporary product `CODEX-BLOCK-491969` with one temporary `wa_order_items` reference. Deleting through the in-page confirmation returned the expected blocker message, `This product is already used by orders or stock reservations. Archive it instead of deleting it.`, the product remained visible, and no browser warnings or errors were recorded. The temporary order, order item, route placement, and product were removed through the protected service-role cleanup, and the UI reloaded back to `8 products` with no smoke code. Product, variant, option, and option-value delete blockers are covered locally by `npm run test:wa` (`17/17`). Next authorized Business-tab work package is Checkout Settings. |
| 2026-07-20 | Implemented the live Business Checkout Settings workspace.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Checkout Settings now loads live business checkout policy, bot prompt settings, delivery areas, pickup locations, and payment methods from the authorized admin details adapter. The UI saves fulfillment policy, minimum order amount, owner approval, checkout prompts, confirmation copy, runtime skip behavior, delivery areas, pickup locations, and payment methods through protected admin APIs with visible loading/success/error feedback and in-page delete confirmations. The backend rejects saving checkout when both delivery and pickup are disabled. The Flow Manager preview boundary now honors explicit live form actions so connected partial routes can submit while future mutations stay blocked. Release marker advanced to `business-checkout-settings-live-v1`. Local gates passed: `npm run test:wa` (`18/18`), `npm test` (`133/133`), `npm run typecheck`, focused ESLint on touched files, `git diff --check`, and `npm run build`. Full `npm run lint` remains blocked by unrelated existing Prettier/CRLF issues in `.vercel/` and the raw `flow-manager/` clone. Production browser mutation and runtime checkout evidence remain before marking Checkout Settings complete. |
| 2026-07-20 | Production-tested Business Checkout Settings from exact commit `3c2e92b`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Production returned `200` with `X-Connect-Release: business-checkout-settings-live-v1` and capability `business-checkout-settings-live`. Authenticated browser verification on `/connect/admin/businesses/double-a-test-business/checkout` showed the live checkout marker, real Arabic/English prompt values, delivery areas, pickup locations, and payment methods. A reversible minimum-order save changed `0` to `0.01`, showed `Checkout settings saved to the live business.`, survived reload, was restored to `0`, and survived another reload. A temporary payment method `Codex Pay 629498` was created through the UI with `Payment method saved.`, appeared under Payment methods, was deleted through the in-page confirmation with `Payment method deleted.`, and remained absent after reload. No browser warnings or errors were recorded. Remaining Checkout Settings completion proof is running an actual WhatsApp order flow that consumes these saved checkout choices. |
| 2026-07-20 | Added checkout-settings runtime regression coverage.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | `tests/connect/whatsapp/checkout-runtime-settings.test.ts` now drives the real `processIncomingMessage` checkout path from cart checkout through customer name, fulfillment, delivery area, address, payment, and notes, using a test-only business checkout settings override. The test asserts the runtime response bodies, delivery rows, delivery fees, and payment labels come from business checkout and bot prompt settings rather than hardcoded defaults. Local gates passed: focused checkout runtime test via `npm test -- tests/connect/whatsapp/checkout-runtime-settings.test.ts` (`116/116` because the project script also expands the full TypeScript test glob), `npm run test:wa` (`18/18`), `npm run typecheck`, focused ESLint on touched files, and `git diff --check`. Remaining Checkout Settings completion proof is a real production WhatsApp order flow consuming the saved deployed checkout choices. |
