...
| 2026-07-17 | Connected the exact Flow Manager business Live Test route to the real admin-authorized WhatsApp services. It now reads the selected Supabase connection, can send the approved `hello_world` template to open a compliant test conversation, can send `/restart` inside an open customer-service window, and refreshes sanitized real message events. Added reusable template sending to the shared Meta sender and extended the existing audited demo-send API without hardcoding the test recipient.                         | Deployment `dpl_7oBVoRU3GvrSS4dUCKZ9PNjyu8rv` rendered the real current connection and historical provider events. Its first production click test exposed the partial-page Future guard intercepting the newly connected send action; the boundary now permits only explicitly connected controls while preserving the partial/Future label and blocking unfinished mutations. Typecheck, scoped lint, all 42 main tests, all 14 reliability tests, production build, and `git diff --check` pass; redeployment and provider roundtrip remain. |
