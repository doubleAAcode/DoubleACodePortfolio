...
- [~] Products now read and mutate real products, category assignment,
      stock/availability, and WhatsApp route placement through the protected
      admin adapter. Product option/value, variant, custom-field, media upload,
      production smoke, and delete-blocker evidence remain before completion.
...
| 2026-07-20 | Implemented the first live Products slice inside the Businesses workspace.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | The exact business tab no longer imports Lovable mock products; it reads real `catalogProducts`, `catalogCategories`, `catalogGroups`, `catalogGroupValues`, `productGroupValues`, product option counts, variants, and custom fields from the authorized business details. Operators can create/edit products, set category, copy, image URL, price, stock, active/sellable status, and required WhatsApp route placement through `save_admin_product`; unsafe deletes go through an in-page confirmation and `delete_admin_product`. Release marker updated to `business-products-live-v1`. Local gates, production deployment, browser mutation smoke, and deeper option/variant/media controls remain before marking Products complete. |
