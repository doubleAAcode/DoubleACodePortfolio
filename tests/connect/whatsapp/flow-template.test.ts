...
import { validateFlow } from "../../../src/features/connect/shared/flow-validation.ts";
import {
  canonicalFlowToRuntimeFlow,
  convertLegacyVisualFlowToCanonical,
} from "../../../src/features/connect/shared/flow-document.ts";
import { validateFlowForEditor } from "../../../src/features/connect/shared/flow-editor.ts";
import {
  addVisualNode,
...
test("official admin templates validate and preserve intended commerce scope", () => {
  ...
  assert.equal(
    ecommerce.nodes.some((node) => node.type === "ORDER_CONFIRMATION"),
    true,
  );
  assert.equal(
    restaurant.nodes.some((node) => node.type === "ORDER_CONFIRMATION"),
    true,
  );
  assert.equal(
    greeting.nodes.some((node) => node.type === "ORDER_CONFIRMATION"),
    false,
  );
  assert.equal(
    greeting.nodes.some((node) => node.type === "CATEGORY_SELECT"),
    false,
  );
  assert.equal(
    greeting.edges.some((edge) => edge.from === "store_info"),
    false,
  );
...
test("greeting template clone preserves start options and targets in the visual builder", () => {
  ...
  assert.equal(
    canonicalRuntime.edges.some(
      (edge) =>
        edge.from === "start" && edge.to === "store_info" && edge.condition === "store_info",
    ),
    true,
  );
  assert.deepEqual(
...
test("ecommerce template clone preserves protected purchase pipeline nodes", () => {
  ...
  assert.equal(
    validateFlow({ ...ecommerce, visualFlow: legacyVisual }, { mode: "publish" }).ok,
    true,
  );
  assert.equal(compiled.ok, true);
  ...
  assert.equal(
    compiled.flow?.nodes.find((node) => node.id === "custom_fields")?.type,
    "CUSTOM_FIELDS",
  );
...
test("image reply option preserves media and can return to the previous options message", () => {
  ...
  assert.equal(
    canonical.nodes.find((node) => node.id === "iphone_price_image")?.type,
    "IMAGE_MESSAGE",
  );
  assert.equal(
      ...
      nodes: [
        {
          ...
          config: {
            mediaUrl: "",
            mediaCaption: { en: "Missing image", ar: "" },
            messageBehavior: "end",
          },
