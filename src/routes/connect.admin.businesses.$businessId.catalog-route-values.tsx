...
export const Route = createFileRoute("/connect/admin/businesses/$businessId/catalog-route-values")({
...
function AdminCatalogRouteValuesPage() {
  ...
  const selectedRouteValueIds = useMemo(() => new Set(values.map((value) => value.id)), [values]);
          ...
          <section className="rounded-lg border border-border bg-surface/60 p-4">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              {values.length ? (
                values.map((value) => {
                  const productCount =
                    details?.productGroupValues.filter((entry) => entry.group_value_id === value.id)
                      .length ?? 0;
                  return (
                    <div
                      key={value.id}
...
function ValueEditor({
  ...
  return (
    <section className="rounded-lg border border-border bg-surface/60 p-4">
      <h2 className="font-display text-lg font-semibold">
        {form.id ? "Edit value" : "Create value"}
      </h2>
