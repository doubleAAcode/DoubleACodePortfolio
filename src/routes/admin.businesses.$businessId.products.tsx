...
function AdminProductsPage() {
  ...
  const productOptionValues = useMemo(
    () =>
      [...(details?.productOptionValues ?? [])]
        ...
        .sort((a, b) => a.sort_order - b.sort_order || a.value_english.localeCompare(b.value_english)),
  ...
  const productCustomFields = useMemo(
    () =>
      [...(details?.productCustomFields ?? [])]
        ...
        .sort((a, b) => a.sort_order - b.sort_order || a.label_english.localeCompare(b.label_english)),
  ...
  async function deleteOption(option: WaProductOptionRow) {
    ...
    if (optionForm.id === option.id) setOptionForm({ ...emptyOption, product_id: selectedProductId });
  }
      ...
      {products.map((product) => {
        ...
        return (
          <div key={product.id} className="rounded-lg border border-border bg-surface/60 p-4">
            <button type="button" className="block w-full text-left" onClick={() => onEdit(product)}>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              <button type="button" className="studio-button-secondary" onClick={() => onEdit(product)}>
                Edit product
              </button>
              <button
                type="button"
        ...
        <div className="mt-4 max-h-[560px] space-y-4 overflow-y-auto pr-1">
          {routeValueGroups.length ? (
            routeValueGroups.map((group) => (
              <div key={group.route.id} className="rounded-md border border-border bg-background p-3">
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        ...
        <div className="space-y-4">
          <div className="rounded-md border border-border bg-surface/60 p-3">
            <h4 className="font-medium">{optionForm.id ? "Edit option group" : "Create option group"}</h4>
            <div className="mt-3 grid gap-3 md:grid-cols-2">
              <TextField
          ...
          <div className="rounded-md border border-border bg-surface/60 p-3">
            <h4 className="font-medium">{optionValueForm.id ? "Edit option value" : "Create option value"}</h4>
            <div className="mt-3 grid gap-3 md:grid-cols-2">
              <SelectField
                label="Option group"
              ...
              <TextField
                ...
                onChange={(value) => onOptionValueChange({ ...optionValueForm, value_english: value })}
              />
              <TextField
                label="Value label AR"
                value={optionValueForm.value_arabic}
                dir="rtl"
                onChange={(value) => onOptionValueChange({ ...optionValueForm, value_arabic: value })}
              />
            </div>
            <TextField
              label="Image URL"
                            ...
                            <input
                              ...
                              onChange={(event) => {
                                const nextIds = event.target.checked
                                  ...
                                  : variantForm.selected_option_value_ids.filter((id) => id !== value.id);
                                onVariantChange({
                                  ...variantForm,
  ...
  return (
    <section className="rounded-lg border border-border bg-background p-4">
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        ...
        <div className="rounded-md border border-border bg-surface/60 p-3">
          <h4 className="font-medium">{customFieldForm.id ? "Edit question" : "Create question"}</h4>
          <div className="mt-3 grid gap-3 md:grid-cols-2">
            <SelectField
              label="Question type"
            ...
            <TextField
              ...
              onChange={(value) => onCustomFieldChange({ ...customFieldForm, label_english: value })}
            />
            <TextField
            ...
            <Toggle
              ...
              onChange={(checked) => onCustomFieldChange({ ...customFieldForm, is_required: checked })}
            />
          </div>
          <TextField
...
function parseChoices(text: string): AdminProductCustomFieldInput["choices"] {
  const choices = text
    ...
    .filter(Boolean)
    .map((choice, index) => ({
      id: choice.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") || `choice-${index + 1}`,
      labelEnglish: choice,
      labelArabic: choice,
    }));
