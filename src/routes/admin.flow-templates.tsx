...
import { getFlowTemplates } from "@/lib/whatsapp/admin-client";
import type { FlowTemplateRow } from "@/lib/whatsapp/flow-template-store.server";
...
function FlowTemplatesPage() {
  ...
  const showingTemplateDetails = pathname.replace(/\/+$/, "") !== "/admin/flow-templates";
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        ...
        <Link to="/admin/flow-templates/new" className="studio-button-primary w-fit">
                ...
                templates.map((template) => (
                  <tr key={template.id} className="border-b border-border/70 last:border-0">
                    <td className="px-4 py-4">
                      <Link
                        to="/admin/flow-templates/$templateId"
