...
import type { AdminBusinessDetails } from "@/features/connect/shared/admin-store.server";
import type {
  BusinessFlowDetails,
  FlowTemplateRow,
...
import { VisualFlowBuilderEditor } from "./connect.admin.businesses.$businessId";

export const Route = createFileRoute("/connect/admin/businesses/$businessId/flow-builder")({
  component: BusinessFlowBuilderPage,
});

const approvedTemplateCategories = new Set(["ECOMMERCE", "RESTAURANT", "GREETING_STORE_INFO"]);
...
function BusinessFlowBuilderPage() {
  ...
  const load = useCallback(
    async (preferredVersionId?: string, label = "loading") => {
      ...
      try {
        ...
        setOrderConfirmationArabic(
          nextAdminDetails.business.order_confirmation_message_arabic || "",
        );
  ...
  const baseFlowForCompile = selectedVersion?.flow_json ?? createDefaultFlowDefinition("ECOMMERCE");
  const compiled =
    !loading && visualFlow
  ...
  async function cloneSelectedTemplate() {
    ...
    const latest = await getBusinessFlowDetails(businessId);
    return (
      latest.versions.find((version) => version.status === "DRAFT")?.id ??
      ...
      <header className="shrink-0 border-b border-border pb-2">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center justify-end gap-2">
            ...
            >
              {!selectedVersionId && visualFlow ? (
                <option value="">Unsaved template flow</option>
              ) : null}
        ...
        <div className="mt-2 flex min-h-6 flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground">
          <span className="font-medium text-foreground">
            {flowEditingTitle(
              ...
              !selectedVersionId && Boolean(visualFlow),
            )}
          </span>
          <span>
            {flowEditingBase(
...
function FlowBuilderMoreMenu({
  ...
  return (
    <details className="relative">
      <summary className="studio-button-secondary h-9 cursor-pointer list-none">More</summary>
