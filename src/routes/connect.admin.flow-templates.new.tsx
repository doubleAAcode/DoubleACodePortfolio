...
import { createFlowTemplate } from "@/features/connect/shared/admin-client";
import {
  createDefaultFlowDefinition,
  type FlowCategory,
} from "@/features/connect/shared/flow-template-types";

export const Route = createFileRoute("/connect/admin/flow-templates/new")({
...
function NewFlowTemplatePage() {
  ...
  const [json, setJson] = useState(
    JSON.stringify(createDefaultFlowDefinition("ECOMMERCE"), null, 2),
  );
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      window.location.href = `/connect/admin/flow-templates/${details.template.id}`;
    } catch (err) {
