...
export function DashboardHome() {
  ...
  const deliveryReady =
    !data.business.allow_delivery || data.deliveryAreas.some((area) => area.is_active);
  const pickupReady =
    !data.business.allow_pickup || data.pickupLocations.some((location) => location.is_active);
  ...
  const steps = [
    ...
    },
    {
      ...
      detail: hasVariantCatalog
        ? "Sellable choices are configured"
        : "Use this for size, color, or flavor",
    ...
    },
    {
      ...
      detail: checkoutReady
        ? "Payments and fulfillment are ready"
        : "Add payment and delivery or pickup options",
      done: checkoutReady,
...
function AttentionRow({
  ...
  return (
    <div className="flex items-center justify-between gap-4 border-b border-border pb-2 last:border-0 last:pb-0">
      ...
      <span className={tone === "warn" ? "font-medium text-destructive" : "font-medium"}>
        {value}
      </span>
    </div>
  );
