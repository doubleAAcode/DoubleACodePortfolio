...
async function enterVisualNode(
  ...
  if (carriedResponses.length >= MAX_AUTOMATIC_RUNTIME_TRANSITIONS) {
    ...
    await timedSaveConversationSession(
      timingContext,
      "visual_transition_limit_session_save",
      session,
      now,
    );
    return [
  ...
  if (!node) {
    await timedSaveConversationSession(
      timingContext,
      "visual_missing_node_session_save",
      session,
      now,
    );
  ...
  if (node.type === "MESSAGE") {
    ...
    if (next) {
      return enterVisualNode(
        baseSession,
        flow,
        next,
        now,
        [...carriedResponses, response],
        timingContext,
      );
    }
  ...
  return {
    ...
    flowVariables: {
      ...
      humanHandoff: {
        status: "paused",
        nodeId,
        pausedAt: getRecord(session.flowVariables.humanHandoff)?.pausedAt ?? now.toISOString(),
      },
    },
  };
}
  ...
  if (node.type === "CATEGORY_SELECT") {
    return [
      ...
      ...(await measureOptionalConversationPhase(
        timingContext,
        "response.browse_group_selection",
        () =>
          browseGroupSelectionResponse(
            nextSession,
...
async function listConfiguredCatalogGroups(
  ...
  const groups = await measureOptionalConversationPhase(
    timingContext,
    "catalog.active_groups_load",
    ...
    const group =
      route.source === "categories"
        ...
        : groups.find(
            (entry) =>
              entry.source === "custom" &&
              (entry.slug === route.groupSlug ||
                entry.slug === route.key ||
                entry.id === route.key),
          );
    if (!group) continue;
...
async function productSelectionResponse(
  ...
  return [
    {
      type: "list",
      body: t(language, `Choose a product from ${browseName}:`, `اختر منتجا من ${browseName}:`),
...
async function pickProductVariantFromList(
  ...
  for (const variant of variants) {
    const label = await variantChoiceLabel(variant, language);
    const values = [variant.id, variant.sku, label, `${label} - ${formatPrice(variant.price)}`].map(
      normalize,
    );
...
async function variantChoiceLabel(variant: StoreProductVariant, language: ConversationLanguage) {
