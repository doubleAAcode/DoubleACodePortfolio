...
export function compileVisualFlowToRuntimeFlow(
  ...
  const legacyOptionsSource =
    ...
    normalizedVisualFlow.nodes.find(
      (node) => node.type === "START" && node.config.menuOptions?.length,
    ) ??
    normalizedVisualFlow.nodes.find(
...
export function validateVisualFlow(visualFlow: VisualFlowDefinition): FlowValidationResult {
  ...
  for (const node of visualFlow.nodes) {
    ...
    if (!outgoing && !isTerminalVisualNode(node) && !protectedCommerceVisualTypes.has(node.type)) {
      issues.push(
        warning(
          "VISUAL_DEAD_END",
    ...
    if (node.type === "PRODUCT_DETAILS") {
      const invalidTarget = outgoingEdges
        ...
        .find(
          (target) =>
            target && target.type !== "CART_REVIEW" && !isLegacyProtectedCommerceNode(target),
        );
      if (invalidTarget) {
        issues.push(
          error(
...
function isTerminalVisualNode(node: VisualFlowNode) {
  return (
    node.type === "END" || node.type === "HUMAN_HANDOFF" || node.config.messageBehavior === "end"
  );
}
