...
Current compatibility adapters live in `src/features/connect/shared/flow-document.ts`:
...
Canonical validation lives in `src/features/connect/shared/flow-validation.ts` and has explicit modes:
